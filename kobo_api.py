...
def fetch_kobo_data(
    ...
    # Hanya ambil kolom yang relevan (sesuai mapping), buang sisanya
    df = df[list(dict.fromkeys(column_map.values()))]  # dedupe order-preserving
...
def load_csv_fallback(uploaded_file, column_map: dict) -> pd.DataFrame:
    ...
    df = df[list(dict.fromkeys(column_map.values()))]
    return df.reset_index(drop=True)
